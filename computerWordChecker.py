        ...
        for length in range(1, len(letters)+1):
            ...
            for spot in range(len(board)):
                if spot not in occupied:
                    ...
                    row = spot//BOARDSIZE
                    column = spot % BOARDSIZE + 1    # first spot is already considered
                    spaceCount = 1
                    ...
                    while spaceCount != length:
                        ...
                        if column != BOARDSIZE:
                            ...
                            if (row*BOARDSIZE + column) not in occupied:
                                # if spot is occupied, move to next column
                                spaceCount += 1
                                spaceCombo.append(row*BOARDSIZE + column)
                    ...
                    if len(spaceCombo) == length and CRC.conformsBetterRules(spaceCombo, attachments):
                        ...
                        # does not append in the escape case
            ...
            if length != 1:
                ...
                for spot in range(len(board)):
                    if spot not in occupied:
                        row = spot//BOARDSIZE + 1
                        column = spot % BOARDSIZE
                        spaceCount = 1
                        spaceCombo = [spot]
                        while spaceCount != length:
                            if row != BOARDSIZE:
            ...
            # row words first, column words second
            ...
            for letterCombo in letterCombos:
                for spaceCombo in spaceCombos[0]:
                    ...
                    if self.comboWorks(letterCombo, spaceCombo, board, occupied, dictionary, True):
                        workingCombos.append(
                            [letterCombo, spaceCombo, self.allCombos])
